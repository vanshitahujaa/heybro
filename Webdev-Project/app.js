...
// Fetch user data from users.txt
app.get('/api/user', (req, res) => {
    ...
    fs.readFile(path.join(__dirname, 'users.txt'), 'utf8', (err, data) => {
        ...
        const user = users.find(u => u.includes(`Username: ${username}`));
        ...
        const emailMatch = user.match(/Email:\s*(.*)/);
        ...
        if (userNameMatch && emailMatch) {
            ...
            return res.json(userInfo); // Send user data as JSON
        } else {
